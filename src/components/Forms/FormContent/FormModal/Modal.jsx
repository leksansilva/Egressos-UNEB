...
export function Modal({ open, closeModal, children, size }) {
  ...
  useEffect(() => {
    setClass(
      `relative bg-brand-500 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-${
        size ? `${size}` : "lg"
      } sm:w-full`
    );
  }, [size]);
      ...
      <div onClick={closeModal} className="relative z-10">
        ...
        <div className="fixed z-20 inset-0 overflow-y-auto">
          <div className="flex items-end sm:items-center justify-center min-h-full p-4 text-center sm:p-0">
            <div onClick={stopPropagation} className={useClass}>
